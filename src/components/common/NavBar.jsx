...
const NavBar = () => {
  ...
  return (
    <>
        ...
        <div className="hidden nav-item md:flex items-center justify-between w-120">
          {navItem.map((item) => (
            <NavLink
              ...
              key={item.title}
              className={({ isActive }) =>
                `hover:text-shadow-white hover:-translate-y-0.5 transition-all duration-200 ease-in-out  text-white ${isActive ? "border-b" : ""}`
              }
            >
              {item.title}
            </NavLink>
      ...
      <div
        className={`open-div md:hidden absolute right-0 z-40  bg-[#2a4f7a]   rounded-bl-2xl w-[65%] shadow-[0_8px_30px_rgba(0,0,0,0.3)] overflow-hidden transition-all duration-600 ease-in-out ${open ? "max-w-xs   opacity-100" : "max-w-0 opacity-0 pointer-events-none"
          }`}
      >
        <div className="flex flex-col gap-1 py-4 px-6">
          {navItem.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `py-3 px-4 rounded-lg text-[15px] font-medium transition-all duration-200 ${isActive
